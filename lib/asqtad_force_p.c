...
void
QOP_asqtad_force_multi_asvec(QOP_info_t *info, QOP_GaugeField *gauge,
			     QOP_Force *force, QOP_asqtad_coeffs_t *coef,
			     REAL eps[], QDP_ColorVector *in_pt[], int nsrc);
		       ...
		       REAL eps[], QOP_ColorVector *in_pt[], int nsrc)
{
  ...
  QOP_asqtad_force_multi_qdp(info, gauge, force, coef, eps, x, nsrc);
			   ...
			   REAL eps[], QDP_ColorVector *in_pt[], int nsrc)
{
  ...
  if( nsrc < QOP_asqtad_ff.fnmat_src_min ) {
    ...
    for(int j=0; j<nsrc; j+=vl) {
      int ns = nsrc-j;
      if(ns>vl) ns = vl;
      QOP_asqtad_force_multi_asvec(&tinfo, gauge, force, coef, eps+j, 
				      in_pt+j, ns);
      info->final_sec += tinfo.final_sec;
      info->final_flop += tinfo.final_flop;
    }
  } else {
    QOP_asqtad_force_multi_fnmat(info, gauge, force, coef, eps, in_pt, nsrc);
  }
