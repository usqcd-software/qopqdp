...
#define NC QDP_get_nc(flw->links[0])
  ...
  for(int i=0; i<n; i++) {
    for(int mu=0; mu<4; mu++) {
      ...
      QDP_D_eq_sD(ys[mu], y2[mu], QDP_neighbor[mu], QDP_forward, all);
    }
    for(int mu=0; mu<4; mu++) {
...
#define NC QDP_get_nc(flw->links[0])
  ...
  for(int mu=0; mu<4; mu++) {
    QDP_M_eq_M_times_Ma(t, links[mu], deriv[mu], all);
    QDP_M_eq_antiherm_M(deriv[mu], t, all);
    QDP_M_peq_M(force[mu], deriv[mu], all);
