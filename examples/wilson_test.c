...
start(void)
{
  ...
  for(sti=0; sti<stn; sti++) {
    st = sta[sti];
    if(QOP_wilson_invert_set_opt("st", st)==QOP_FAIL) continue;
    for(nsi=0; nsi<nsn; nsi++) {
      ns = nsa[nsi];
      if(QOP_wilson_invert_set_opt("ns", ns)==QOP_FAIL) continue;
      for(nmi=0; nmi<nmn; nmi++) {
	nm = nma[nmi];
	if(nm==0) nm = ns;
	if(QOP_wilson_invert_set_opt("nm", nm)==QOP_FAIL) continue;
